...
public class PrestamoEstudiante implements ComportamientoPrestamo {
  ...
  public RealmList<Libro> prestar(RealmList<Libro> libros) {
    ...
    for (Libro libro : libros) {
      libroP =
          librosRepository.find(
              libro.getIsbn(), EstadoLibro.SEMI_NUEVO, DisponibilidadLibro.DISPONIBLE);
      if (libroP == null) {
        libroP =
            librosRepository.find(
                libro.getIsbn(), EstadoLibro.NUEVO, DisponibilidadLibro.DISPONIBLE);
        if (libroP == null) {
          libroP =
              librosRepository.find(
                  libro.getIsbn(), EstadoLibro.VIEJO, DisponibilidadLibro.DISPONIBLE);
        }
      }
